      ...
      >
        ...
        <div className="hidden md:flex space-x-6 font-inter ml-4 mr-6">
          {sectionLinks.map((link) => (
            <a
              ...
              href={`#${link}`}
              aria-label={`#${link}`}
              className="text-gray-300 font-inter font-bold transition-transform duration-300 ease-out transform hover:scale-105 hover:text-success hover:shadow-md hover:underline"
            >
              {link}
            </a>
          ))}
        ...
        {isOpen && (
          <div className="fixed inset-0 flex justify-end md:hidden bg-black bg-opacity-30">
            ...
            {/* Side Menu Content */}
            <div
              className={`bg-gradient-to-r from-neutral to-base-100 text-white w-2/3 h-screen z-50 fixed right-0 top-0 flex flex-col justify-center items-center transition-transform ease-in-out duration-300 ${
